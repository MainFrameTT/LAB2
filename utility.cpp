...
// checkers
bool checker_bool() // Ïðîâåðêà íà áóëåâîå çíà÷åíèå
{
    bool in;
    while (1) {
        if (cin >> in)
        {
            cin.ignore(1024, '\n'); // Îòñåêàòü âñ¸, ÷òî äàëüøå ïðîáåëà
            return(in);
        }
        else
        {
...
int checker() // Ïðîâåðêà íà ïîëîæèòåëüíîå ÷èñëî 
{
    int in;
    while (1) {
        if ((cin >> in) && (in >= 0))
...
int checker(int a, int b) // Ïðîâåðêà íà ÷èñëî â èíòåðâàëå îò a äî b âêëþ÷èòåëüíî
{
    int in;
    while (1) {
        if ((cin >> in) && (in >= a) && (in <= b))
...
// Ôóíêöèè âûâîäà, äîáàâëåíèÿ, ðåäàêòèðîâàíèÿ è óäàëåíèÿ îáúåêòîâ òðóá
...
// Ôóíêöèè âûâîäà, äîáàâëåíèÿ, ðåäàêòèðîâàíèÿ è óäàëåíèÿ îáúåêòîâ ÊÑ
...
{
    ifstream exist(name_file + ".txt"); // Ïðîâåðêà ñóùåñòâîâàíèÿ ôàéëà
    bool file_exist;
...
void load_data(GTN_data& gtn_struct)
{
    ...
    if (ining.is_open())
    {
        int type = 0;
        int id = 0;
        while (1) // Ñ÷èòûâàíèå äàííûõ èç ôàéëà äî êîíöà ôàéëà
        {
            if (ining.eof()) break;
            ining >> type;
            if (type == 1) {
